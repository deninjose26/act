...
# Initialize Together client using session state for API key persistence
def get_together_client(api_key):
    if not api_key:
        st.warning("API key not found. Please add a valid Together API key in the sidebar.")
        return None
...
def main():
    ...
    with st.sidebar:
        ...
        api_key = st.text_input("Together API Key", 
                               ...
                               type="password",
                               key="api_key_input",
                               on_change=lambda: setattr(st.session_state, 'api_key', st.session_state.api_key_input))
        
        st.header("Model Settings")
        model = st.selectbox(
            "Select Model", 
            ["deepseek-ai/DeepSeek-R1-Distill-Llama-70B-free", "deepseek-ai/DeepSeek-V3"],
            index=0
        )
        temperature = st.slider("Temperature", min_value=0.0, max_value=1.0, value=0.3, step=0.1)
        max_tokens = st.slider("Max Tokens", min_value=500, max_value=4000, value=2000, step=100)
    
    # Get Together client based on current API key
        ...
        if uploaded_file is not None:
            try:
                ...
                if st.button("Process CSV File"):
                    if client is None:
                        st.error("Please enter a valid Together API key in the sidebar to proceed.")
                    else:
